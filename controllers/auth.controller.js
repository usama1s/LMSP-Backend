...
module.exports = {
  ...
  async register(req, res) {
    try {
      const { email, password, role, marital_status, country, profile_image_type, organization, designation, qualification, register_date, register_id, admin_type, status, employee_id, first_name, last_name, profile_image } = req.body;
      ...
      const fileName = `profile_image${Date.now()}.${fileExtension || defaultExtension}`;
      const profile_image_path = path.join(__dirname, '../uploads', fileName);
  
      await fs.writeFile(profile_image_path, buffer);
  
      const registrationResult = await authService.register(
        email, password, role, marital_status, country, organization, designation,
        qualification, register_date, register_id, admin_type, status, employee_id,
        profile_image_path, first_name, last_name,
      );
